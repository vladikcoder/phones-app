...
const PhoneService = {
  getPhones(renderCallback, wrapperElement, returnParsed) {
    let xhr = new XMLHttpRequest();

    xhr.open('GET', 'https://learn.javascript.ru/task/phones-list/phones-async/phones.json');
    // xhr.open('GET', 'https://vladikcoder.github.io/phones-app/phones/phones.json');
    xhr.send();
    ...
    xhr.onreadystatechange = function () {
      ...
      if (!this.status) {
        ...
        try {
          returnParsed = JSON.parse(this.responseText);
          PhoneService.getPhones.cached = returnParsed;
          renderCallback(wrapperElement, returnParsed);
        } catch (e) {
          console.log(`Invalid answer: ${e.message}`);
        }
      }
    };
  },
  ...
  getById(phoneId) {
    return phoneDetails.find(phoneObj => phoneObj['id'] === phoneId);
  },
};
